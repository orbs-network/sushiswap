...
const useWrap = () => {
  ...
  const {
    state: { token0, swapAmount },
  } = useDerivedStateTwapSwap()

 const value = swapAmount?.numerator.toString() || ''


  const { data: simulation } = useSimulateContract({
    address: token0?.wrapped.address as Address,
    abi: [{
      "constant": false,
      "inputs": [],
      "name": "deposit",
    ...
    value,
    query: { enabled: Boolean(token0 && value && address) },
  })

    const {sendTransaction, isPending} =  useSendTransaction()
    const wrap =  useCallback(
      () => {
        sendTransaction(simulation?.request)
      },
...
export const TwapSwapTradeReviewDialog: FC<{
  children({
    error,
    isSuccess,
  ...
  const {wrap, isPending: wrapPending} = useWrap()

  const [slippagePercent] = useSlippageTolerance()
  const { data: trade, isFetching } = useTwapSwapTrade()
