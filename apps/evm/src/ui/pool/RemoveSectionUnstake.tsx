...
import { useMasterChef, useMasterChefWithdraw } from '@sushiswap/wagmi'
...
export const RemoveSectionUnstake: FC<{ pool: Pool }> = ({ pool }) => {
...
export const _RemoveSectionUnstake: FC<AddSectionStakeProps> = withCheckerRoot(
  ({ chainId, pool, incentiveType, farmId }) => {
    const [value, setValue] = useState('0')
    const { liquidityToken } = useExtendedPool({ pool })
